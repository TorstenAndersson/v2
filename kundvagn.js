...
function selectChanged(sender) {
	...
	if (document.querySelector(".totalText") != null) document.querySelector(".sumNumber").innerText = (Math.round(Array.from(document.querySelectorAll(".totalText")).reduce((x, y) => +x.innerText.slice(0, -3).replace(",", ".") + +y.innerText.slice(0, -3).replace(",", ".")) * 100)/100).toString().replace(".", ",") + " kr";
