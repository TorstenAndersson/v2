function pageLoaded() {
	if (localStorage["cart"] !== "{}" && localStorage["cart"] !== undefined) {
		...
		document.querySelector(".sumNumber").innerText = Array.from(document.querySelectorAll(".totalText")).reduce((x, y) => +x.innerText.slice(0, -3).replace(",", ".") + +y.innerText.slice(0, -3).replace(",", ".")).replace(".", ",") + " kr";
...
function selectChanged(sender) {
	...
	if (document.querySelector(".totalText") != undefined) document.querySelector(".sumNumber").innerText = Array.from(document.querySelectorAll(".totalText")).reduce((x, y) => +x.innerText.slice(0, -3).replace(",", ".") + +y.innerText.slice(0, -3).replace(",", ".")).replace(".", ",") + " kr";
