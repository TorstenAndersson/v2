...
for phone in sorted(os.listdir("imgs/Johans Skal/"), reverse=True):
    ...
    print(str(response.find("span", {"data-spec": "released-hl"}).text.replace(" ", "")))#[9:13]))
        ...
        for i in range(len(product["perspectives"])):
            perspectives += '''
                    <div class="smallProductFrame">
                        <img class="smallProductImg" src="/imgs/''' + product["name"] + "/" + product["name"] + " " + product["variants"]["Storlek"][0] + " " + product["perspectives"][i] + '.webp" width="100" height="100" alt="' + product["name"] + ''' Perspective: ''' + product["perspectives"][i] + '''" onmouseover="smallImgHovered(this)">
                    </div>'''
    except KeyError:
        pass
