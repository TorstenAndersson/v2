...
function buy(sender) {
	...
	try {
		...
		localStorage["cart"] = localStorage["cart"].slice(0, -(2 + (Object.values(JSON.parse(localStorage["cart"])).reduce((a, b) => parseInt(a) + parseInt(b))/10).toString().length)) + JSON.stringify(parseInt(JSON.parse(localStorage["cart"])[sender.parentElement.parentElement.firstElementChild.innerText + ">" + variant.slice(0, -2)].slice(0)) + 1) + '"}';
	} catch {
