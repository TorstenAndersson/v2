	...
	if (smallImgs.length !== 0) {
		for (i2 in smallImgs) {
			if (decodeURI(smallImgs[i2].src.split("%20")).split(",").some((element) => sender.innerText.includes(element)))
				...
				splitSource.splice(splitSource.lastIndexOf(encodeURI(smallImgs[i2].src.split("%20").filter((element) => sender.innerText.includes(element))[0])), 1, encodeURI(sender.value));
