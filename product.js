	...
	if (smallImgs.length !== 0) {
		for (i2 in smallImgs) {
			if (decodeURI(smallImgs[i2].src.split("%20")).split(",").some((element) => sender.innerText.includes(element)))
				...
				smallImgs[i2].src = smallImgs[i2].src.replace(encodeURI(decodeURI(smallImgs[i2].src).split(" ").filter((element) => sender.innerText.includes(element))[decodeURI(smallImgs[i2].src).split(" ").filter((element) => sender.innerText.includes(element)).length - 1]), encodeURI(sender.value));
			}
