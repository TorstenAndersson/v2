	...
	if (smallImgs.length !== 0) {
		for (i2 in smallImgs) {
			if (decodeURI(smallImgs[i2].src.split("%20")).split(",").some((element) => sender.innerText.includes(element)))
				...
				splitSource.splice(splitSource.lastIndexOf(encodeURI(splitSource.filter((element) => sender.innerText.includes(decodeURI(element)))[0])), 1, encodeURI(sender.value));
