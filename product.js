	...
	if (smallImgs.length !== 0) {
		for (i2 in smallImgs) {
			if (decodeURI(smallImgs[i2].src.split("%20")).split(",").some((element) => sender.innerText.includes(element)))
				...
				smallImgs[i2].src = encodeURI(smallImgs[i2].src.replace((smallImgs[i2].src).split("%20").filter((element) => sender.innerText.includes(element))[smallImgs[i2].src.split("%20").filter((element) => sender.innerText.includes(element)).length - 1]), encodeURI(sender.value));
			}
