...
function variantChanged(sender) {
	...
	if (smallImgs.length !== 0) {
		for (i2 in smallImgs) {
			...
			smallImgs[i2].src = decodeURI(smallImgs[i2].src.replace(encodeURI("Johans Baddräkt " + decodeURI(smallImgs[i2].src).split(" ").filter((element) => sender.innerText.includes(element))[1]), encodeURI("Johans Baddräkt " + sender.value)));
		}
