...
function buy(sender) {
	...
	try {
		...
		localStorage["cart"] = localStorage["cart"].slice(0, -3) + parseInt(JSON.stringify(JSON.parse(localStorage["cart"])[sender.parentElement.parentElement.firstElementChild.innerText + ">" + variant.slice(0, -2)].slice(0)) + 1) + '"}';
	} catch {
